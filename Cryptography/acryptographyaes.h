// ----------------------------------------------------------
/*!
	\headerfile ACryptographyAES
	\title
	\brief Template file wizard/classes/cpp/file.h

	\list
	\li @notice Template file classes/file.h
	\li @copyright Arboreus (http://arboreus.systems)
	\li @author Alexandr Kirilov (http://alexandr.kirilov.me)
	\li @created 11/09/2021 at 18:27:07
	\endlist
*/
// ----------------------------------------------------------

#ifndef ACRYPTOGRAPHYAES_H
#define ACRYPTOGRAPHYAES_H

// System includes

// Precompiled includes
#include <acryptographypch.h>

// Application includes
#include <acryptographydatamodels.h>
#include <acryptographyaesglobal.h>

// Constants and definitions


// Namespace
namespace ARB {

// Class definitions
class ACryptographyAES : public QObject {

	Q_OBJECT

	public:

		explicit ACryptographyAES(
			_A_ENUMS_CRYPTOGRAPHY_AES_TYPE inLevel,
			_A_ENUMS_CRYPTOGRAPHY_AES_MODE inMode,
			_A_ENUMS_CRYPTOGRAPHY_AES_PADDING inPadding = _A_ENUMS_CRYPTOGRAPHY_AES_PADDING::ISO,
			QObject* parent = nullptr
		);
		virtual ~ACryptographyAES(void);

		QByteArray mEncode(
			const QByteArray& inRawText,
			const QByteArray& inKey,
			const QByteArray& inIv = NULL
		);
		QByteArray mDecode(
			const QByteArray& inRawText,
			const QByteArray& inKey,
			const QByteArray& inIv = NULL
		);
		QByteArray mRemovePadding(const QByteArray& inRawText);
		QByteArray mExpandKey(const QByteArray& inKey);

		static QByteArray mCrypt(
			_A_ENUMS_CRYPTOGRAPHY_AES_TYPE inLevel,
			_A_ENUMS_CRYPTOGRAPHY_AES_MODE inMode,
			const QByteArray& inRawText,
			const QByteArray& inKey,
			const QByteArray& inIv = NULL,
			_A_ENUMS_CRYPTOGRAPHY_AES_PADDING inPadding = _A_ENUMS_CRYPTOGRAPHY_AES_PADDING::ISO
		);
		static QByteArray mDecrypt(
			_A_ENUMS_CRYPTOGRAPHY_AES_TYPE inLevel,
			_A_ENUMS_CRYPTOGRAPHY_AES_MODE inMode,
			const QByteArray& inRawText,
			const QByteArray& inKey,
			const QByteArray& inIv = NULL,
			_A_ENUMS_CRYPTOGRAPHY_AES_PADDING inPadding = _A_ENUMS_CRYPTOGRAPHY_AES_PADDING::ISO
		);
		static QByteArray mExpandKey(
			_A_ENUMS_CRYPTOGRAPHY_AES_TYPE inLevel,
			_A_ENUMS_CRYPTOGRAPHY_AES_MODE inMode,
			const QByteArray& inKey
		);
		static QByteArray mRemovePadding(
			const QByteArray& inRawText,
			_A_ENUMS_CRYPTOGRAPHY_AES_PADDING inPadding
		);

	private:

		int pNb;
		int pBlockLength;
		int pLevel;
		int pMode;
		int pNk;
		int pKeyLength;
		int pNr;
		int pExpandedKey;
		int pPadding;
		QByteArray* pState;

		quint8 mGetSBoxValue(quint8 inNum) {return pSBox[inNum];}
		quint8 mGetSBoxInvert(quint8 inNum) {return pRSBox[inNum];}

		void mAddRoundKey(const quint8 inRound, const QByteArray inExpKey);
		void mSubBytes(void);
		void mShiftRows(void);
		void mMixColumns(void);
		void mInvMixColumns(void);
		void mInvSubBytes(void);
		void mInvShiftRows(void);
		QByteArray mGetPadding(int inCurrSize, int inAlignment);
		QByteArray mCipher(const QByteArray& inExpKey, const QByteArray& inPlainText);
		QByteArray mInvCipher(const QByteArray& inExpKey, const QByteArray& inPlainText);
		QByteArray mByteXor(const QByteArray& inIn, const QByteArray& inIv);

		const quint8 pSBox[256] = {
			
		};

		const quint8 pRSBox[256] = {
			
		};

		const quint8 pRCon[14] = {
			0x8d, 0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1b, 0x36, 0x6c, 0xd8, 0xab
		};
};

} // namespace ARB

#endif // ACRYPTOGRAPHYAES_H
